...
var resizePizzas = function(size) {
  ...
  function changePizzaSizes(size) {
    /** Change from the querySelectorAll to the getElementsByClassName API Web call – Better performance
     *  Additionally, remove the dx and newwidth variables from the for statement and save the randomPizzaContainer class selector into a local variable, so the DOM is only touched once per function call.
     */
...
/** Remove the pizzaDiv variable, so only one DOM call is done.*/
...
function updatePositions() {
  ...
  /** Move the document.body.scrollTop so the DOM is only called once */
